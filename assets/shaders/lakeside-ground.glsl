...
vec4 effect(vec4 color, Image image, vec2 uv, vec2 pixel) {
    ...
    // Recede the distant forest only; keep playable grass and lake colors.
    float distant=max(1.0-smoothstep(.12,.21,sampleUV.y),1.0-smoothstep(.08,.16,sampleUV.x));
    surface.rgb=mix(surface.rgb,vec3(.40,.45,.30),distant*.18);

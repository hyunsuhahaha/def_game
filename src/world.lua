    ...
    for _, node in ipairs(self.nodes) do
        ...
        if node.kind == "tree" then
            ...
            if node.burning then
                -- Continuous tree fire is animated by its authored atlas.  Do not
                -- shake the whole tree or emit a repeating combustion beat here.
            else
                node.burnDrawOffset=nil
            end
        end
...
function World:draw(player, actorSource)
    ...
    if not player.introHidden then
        local drawX,drawY=player.x,player.y
        if player.autoAxeRenderPosition then drawX,drawY=player:autoAxeRenderPosition()end
        queue[#queue + 1] = {x=drawX,y=drawY,draw = function() player:draw() end}
    end

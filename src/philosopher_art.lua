...
function Art.channel(mode,game,active,tx,ty,power)
    ...
    stream.tx,stream.ty,stream.power=tx,ty,power or 0
...
function Art.draw(mode)
    ...
    for i=0,count do
        local along=(i*gap+travel)%dist;local p=along/dist
        local wobble=math.sin(v.t*24+i*1.9)*math.min(7,2+dist*.012)
        local x=v.x+nx*along-ny*wobble;local y=v.y+ny*along+nx*wobble
        local frame=(i+math.floor(v.t*18))%6+1;local scale=.20+(v.power or 0)*.055+(i%3)*.018

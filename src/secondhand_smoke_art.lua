...
function Art.draw(mode)
    ...
    for _,cloud in ipairs(clouds) do
        ...
        local phase=cloud.age*FRAME_RATE
        local whole=math.floor(phase)
        local frame=1+(whole%FRAMES)
        local nextFrame=1+(frame%FRAMES)
        local blend=phase-whole
        blend=blend*blend*(3-2*blend)
        local fade=math.min(1,cloud.age*7,(1-progress)*3.2)
        local sx,sy=WORLD_W/CELL_W,WORLD_H/CELL_H
        -- Equal-power alpha crossfade keeps the fog's perceived density while
        -- the authored pixel clusters dissolve into the following frame.
        local currentWeight=(1-blend)^.62
        local nextWeight=blend^.62

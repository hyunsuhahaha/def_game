...
stream = Image.open(ROOT / "assets/effects/smoker-flamethrower-stream-atlas-v2.png").convert("RGBA")
...
for index in range(8):
    ...
    heights = []
    for left, right in ((180,340),(500,660),(820,980)):
        bbox = alpha.crop((left,0,right,768)).getbbox()
        heights.append(bbox[3]-bbox[1])
    assert max(heights) / min(heights) < 1.38, "stream silhouette widened into a cone"
...
print("FLAMETHROWER_ASSETS_OK stream=5120x1536 frames=8 equipment=384x128 silhouette=rolling-column")

...
S, CELL, FUSE_FRAMES, EXPLOSION_FRAMES = 2, 128, 24, 24
...
def explosion(frame: int) -> Image.Image:
    ...
    d = ImageDraw.Draw(im)
    cx, cy = 64, 70
    phase = frame * 7 / (EXPLOSION_FRAMES - 1)
    progress = phase / 7
    if phase < 5:
        radii = [14, 29, 45, 56, 62]
        step = min(3, int(phase))
        blend = phase - step
        radius = radii[step] + (radii[step + 1] - radii[step]) * blend
        colors = [(255, 246, 175, 255), (255, 218, 61, 255), (255, 137, 20, 255), (225, 63, 15, 255)]
        for layer, color in enumerate(reversed(colors)):
            rr = max(5, radius - layer * (7 + phase))
            points = []
            for i in range(24):
                angle = i * math.pi * 2 / 24
                jag = 1 + math.sin(i * 4.7 + phase * .65) * .16
                if i % 4 == 0:
                    jag += .22 + phase * .025
        ...
        if phase <= 2:
            core = round(radius / 3)
            d.ellipse((cx-core, cy-core, cx+core, cy+core), fill=(255, 255, 224, 255))
        for i in range(10):
    ...
    if phase >= 3:
        smoke_age=phase-3
        smoke_alpha=round(235-min(1,max(0,phase-5)/2)*50)
        smoke=(56+round(max(0,phase-5)*8),49+round(max(0,phase-5)*9.5),45+round(max(0,phase-5)*9),smoke_alpha)
        for i in range(9):
            angle=i*math.pi*2/9+.35
            spread=22+smoke_age*9+(i%3)*5
            x=cx+math.cos(angle)*spread
            y=cy-18-smoke_age*7+math.sin(angle)*spread*.48
            size=15+(i*7+round(phase*3))%11
        ...
        if phase < 6:
            d.ellipse((cx-25-phase*2,cy-24-phase*3,cx+25+phase*2,cy+22),fill=(255,94,14,230))
            d.ellipse((cx-14,cy-17-phase*3,cx+14,cy+12),fill=(255,220,48,245))
    for i in range(12):
        angle=i*math.pi*2/12+.17
        distance=(18+phase*10)*(1+(i%3)*.12)
        x=round(cx+math.cos(angle)*distance);y=round(cy+math.sin(angle)*distance*.72)
        size=3 if phase<5 else 2
        d.rectangle((x-size,y-size,x+size,y+size),fill=(255,151 if i%2 else 76,18,255 if phase<6 else 145))
...
def build() -> None:
    ...
    fx_atlas = Image.new("RGBA", (256 * EXPLOSION_FRAMES, 256), (0, 0, 0, 0))
    ...
    for index in range(EXPLOSION_FRAMES):
        canvas=Image.new("RGB",(512,384),(34,66,29));draw=ImageDraw.Draw(canvas)
    ...
    durations=[100]*(4+FUSE_FRAMES)+[33]*EXPLOSION_FRAMES+[100]*4
    gif_frames[0].save(PREVIEW/"bomb-fuse-explosion-v1.gif",save_all=True,append_images=gif_frames[1:],duration=durations,loop=0,optimize=False)

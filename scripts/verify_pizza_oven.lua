...
do
    ...
    -- 그루당 초당 1, 조각당 10 이므로 두 그루면 5초에 한 조각이다.
    for _=1,250 do Oven.update(mode,.02,game)end
    assert(oven.slices==1,"two burning trees over 5s did not bake exactly one slice, got "..oven.slices)
end
...
do
    ...
    assert(math.abs(effects.scoreOvenHeat-1.6)<1e-6,"heat branch total changed")
    assert(math.abs(effects.scoreOvenSliceCost-4.5)<1e-6,"dough branch total changed")
    ...
    local maxed={scoreOvenUnlock=1,scoreOvenRadius=200,scoreOvenHeat=1.6,scoreOvenSliceCost=4.5,
    ...
    assert(math.abs(Oven.heatPerTree(mode)-2.6)<1e-6,"maxed heat wrong")
    assert(math.abs(Oven.sliceCost(mode)-5.5)<1e-6,"maxed slice cost wrong")
...
Oven.load()
print("PIZZA_OVEN_OK fuel=burning_trees_only center_placed=true radius=260 slice_cost=10 tray=6 "..

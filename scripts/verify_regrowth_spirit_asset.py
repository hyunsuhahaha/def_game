...
MODEL = ROOT / "assets/enemies/concepts/regrowth-prism-pedestals-concept-v4-cutout.png"
ATLAS = ROOT / "assets/enemies/arcade/planter-forest-atlas-v4.png"
REPORT = ROOT / "docs/previews/regrowth-totems-v4-build.json"
...
def main():
    ...
    assert colors >= 70, f"material palette collapsed to {colors} colors"
    frames = [pixels[row * 256:(row + 1) * 256, col * 256:(col + 1) * 256]
              for row in range(2) for col in range(6)]
    assert len({frame.tobytes() for frame in frames[:6]}) == 1, "stable pedestal pumps during idle"
    assert len({frame.tobytes() for frame in frames[6:]}) == 1, "stable pedestal pumps during casting"
    assert len({frame.tobytes() for frame in frames}) == 2, "pedestal should change only its cast lighting"
    data = json.loads(REPORT.read_text(encoding="utf-8"))
    assert data["forest"]["frames"] == 12 and data["forest"]["worldWidth"] == 68 and data["forest"]["openCore"]
    for name,width in {"forest":68,"mangrove":70,"madagascar":72,"island":74}.items():
        ...
        assert data[name]["worldWidth"]==width and 206<=data[name]["bodyWidth"]<=214 and data[name]["stableBody"]
    catalog = (ROOT / "src/forest_arcade_catalog.lua").read_text(encoding="utf-8")
    assert 'planter-forest-atlas-v4.png' in catalog and 'planter-island-atlas-v4.png' in catalog and 'cell=256' in catalog and "Placeholder art" not in catalog
    ...
    print(f"REGROWTH_SANCTUM_ASSET_OK body={atlas.width}x{atlas.height}/12 v4=open_core cast=1536x256/6 colors={cast_colors} structure=true mystical=true")

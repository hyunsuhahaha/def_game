...
local a=A.new(true)
assert(#a:getDefinitions()==30,"achievement catalog must contain 30 entries")
local species=0 for _,d in ipairs(a:getDefinitions())do if d.category=="species"then species=species+1 end end
assert(species==16,"all 16 tree species need a 100-tree achievement")
...
for _,map in ipairs({"forest","mangrove","madagascar","island","beginner","greatforest"})do a:recordMapClear(map)end
assert(a:isUnlocked("first_operation") and a:isUnlocked("all_operations"),"operation achievements did not unlock")
local operationSave=A.decode(A.encode(a.data));assert(operationSave.clears.greatforest and operationSave.stats.unique_operations==6,"operation clear save failed")
...
print("ACHIEVEMENTS_OK total=30 species=16 operations=6 rewards=4 popup=queued save=v1")

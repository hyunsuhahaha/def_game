...
for _,command in ipairs(fixture.commands)do if command.op=="draw"then
    ...
    if command.file=="assets/effects/smoker-flamethrower-stream-atlas-v4.png"then atlas=atlas+1 end
end end
...
for index=0,7 do
    ...
    assert(command and command.file=="assets/effects/smoker-flamethrower-stream-atlas-v4.png","stream atlas draw disappeared")
...
for _,entry in ipairs(g.world.billboardQueue)do
    if entry.draw then fixture.reset();entry.draw()
        for _,command in ipairs(fixture.commands)do
            if command.op=="draw"and command.file=="assets/effects/smoker-flamethrower-stream-atlas-v4.png"then queued=true end
        end
    end
end
assert(queued,"flamethrower stream did not enter the upright 2.5D billboard pass")
print("FLAMETHROWER_ART_OK atlas=v4:1536x768x8 equipment=384x128 nearest=true billboard=upright source=captured")

...
assert(topScale==1 and bottomScale==1,"ordinary 2.5D depth scale makes the floor breathe vertically")
...
assert(projection:find("math.floor(screenX+.5),math.floor(screenY+.5)",1,true),"billboard feet are not snapped to screen pixels")
...
print(string.format("CAMERA_25D_OK pitch=.76 inverse=exact depthScale=%.2f stableGround=affine shake=off roll=off billboards=uniform",topScale))

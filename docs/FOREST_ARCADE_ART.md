...
- 폭죽 폭발이 화덕 본체에 닿으면 같은 전용 화실 불꽃이 즉시 켜지고, 타는 나무 한 그루분의 화력으로 4초간 직접 연소한다. 비가 오면 이 직접 연소도 즉시 꺼진다.
...
## 화염방사기 대형 난류 분사 FX (v5)

- 폭죽 다음 원거리 무기인 화염방사기는 런타임 반투명 삼각형 부채를 사용하지 않는다.
  사용자 제공 화염 캡처의 긴 백색 고열핵·황색 중간층·적주황 외곽층을 기준으로
  연소 순간 8장을 각각 별도로 그린다. `smoker-flamethrower-stream-atlas-v5.png`의
  1536×768 원생 셀을 16fps로 순환하며, 매 순간 외곽 화염 혀와 내부 고열 갈래가 모두
  새 형상으로 바뀐다. 고정 원화의 흔들기·회전·봉우리 복제는 사용하지 않는다.
  검수용 모션은 `smoker-flamethrower-stream-v5.gif`에 같은 8프레임으로 보존한다.
...
- 2026-09-02 밸런스 조정으로 강화 전 분사 거리는 250→300, 거리 연구 만렙은
  500→550이 됐다. 아틀라스의 원생 픽셀 밀도는 유지하고 `reach`만 늘려 판정과 표시가
  함께 확장되도록 검수한다.

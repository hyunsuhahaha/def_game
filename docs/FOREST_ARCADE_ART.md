...
- 유출 완료 뒤 바닥 기름은 `oil-puddle-atlas-pixel-v2.png`의 고정된 외곽을 사용한다. 8개 셀을 시간으로 순환하지 않으므로 웅덩이 가장자리와 지면 접점이 흔들리지 않는다.
- 점화 시 `oil-fire-overlay-atlas-pixel-v2.png`의 웅덩이 전체 화염을 한 번만 겹친다. 작은 불꽃을 여러 장 타일링하거나 전체 크기를 펄스시키지 않는다.
- 화염의 뿌리는 기름 표면에 고정하고 위쪽 불꽃·불티·연기만 8프레임에서 움직인다. 기본 반경 140과 범위 강화(+18/단계)는 바닥·화염·피해 판정에 같은 배율을 적용한다.
- `scripts/build_oil_puddle_fire_v2.py`가 256×200 셀, 4×2 아틀라스와 실제 크기/확대 검수 보드를 만든다. 생성 콘셉트는 형태·재질 입력으로만 보존하고 최종 알파·팔레트·앵커는 빌더에서 고정한다.

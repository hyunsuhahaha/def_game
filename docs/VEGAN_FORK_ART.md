...
고정 원화는 `assets/characters/concepts/vegan-woman-model-v3-source.png`, 포크 원화는 `assets/characters/concepts/vegan-fork-model-v1-source.png`다. 최종 런타임 자산은 원화를 그대로 축소하지 않고 `scripts/build_vegan_fork_assets.py`에서 고정 픽셀 그리드와 제한 팔레트로 다시 작성한다.
...
| `vegan-fork-impact-atlas-v1.png` | 96×96, 6프레임 | 접촉 순간의 금속성 별·잎·나무 파편 |
| `vegan-chomp-atlas-v1.png` | 160×160, 8프레임 | 나무를 먹을 때의 우스운 턱·부스러기 강조 |
...
포크 타격으로 나무 HP가 0이 되었을 때만 다음 순서를 실행한다.

1. 포크 접점에서 충격 별과 잎·나무 조각이 튄다.
2. 쓰러진 나무 원본 실루엣이 작아지며 곡선을 따라 캐릭터 입 쪽으로 빨려 온다.
3. 입 근처에서 큰 만화식 `냠` 실루엣과 부스러기가 터진다.
4. `접시 비우기`는 체력과 추가 목재를, `한 그릇 더`는 먹은 직후 공격 속도를 준다.

연출은 공포스러운 포식이나 진지한 피·육편 표현을 쓰지 않는다. 과장된 포크, 별 모양 충격, 잎과 부스러기로 가볍고 우스운 느낌을 유지한다.

## 채택 스킬 구성

- **대왕 포크**: 기본 포크 찌르기의 피해·폭·다중 목표를 강화한다.
- **뷔페용 포크**: 사거리와 찌르기 잔상을 강화하며 6레벨에 두 번째 포크 타격을 추가한다.
- **접시 비우기**: 나무를 먹을 때 회복과 추가 목재를 주며 6레벨에 부스러기 범위 피해를 낸다.
- **한 그릇 더**: 나무를 먹은 직후 공격 속도와 연속 식사 능력을 높인다.
...
- `docs/previews/vegan-fork-runtime-v3-1280.png`와 `vegan-fork-runtime-v3-960.png`를 실제 크기로 확인한다.

두 캡처 모두에서 여성 체형, 네 갈래 포크, 전방 레인, 나무 먹기 효과가 즉시 읽히고 서로 겹쳐 판정을 오해하게 만들지 않아야 한다.

...
`src/upland.lua`는 플레이 영역 전체에 나무를 분산 생성한다. 고정 군락·중앙 공터·운반로 제한은 없다. 연속 생성 위치에 작은 무작위 편차를 더해 줄맞춤과 한쪽 쏠림을 피한다. 단계 전환 시 기존 나무의 위치·HP·상태·보상을 유지한다. 생성량·허용량은 기존 전투 수치를 따르므로 적은 나무 수에서 전역을 밀림처럼 채우지는 않는다.

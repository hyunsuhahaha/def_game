...
크레인 공격 범위: 기본 반경 24·굴림 거리 600, 최대 강화 반경 64·거리 1400. 원통의 표시 크기와 실제 충돌 폭은 같은 반경을 공유한다.

현재 카메라/난이도: 크레인은 흡연자보다 가로·세로 시야가 40% 넓은 추적 시점(기본 줌 ÷1.4)이며 Ctrl+휠은 0.90~1.10배로 제한한다. 전체 맵을 맞추는 자동 축소는 제거했다. 11단계 이상 재생 속도는 기존의 45%, 허용량은 +80그루(중복 가산 없음), 과밀 유예는 기존 연구 효과에 8초를 더한다.

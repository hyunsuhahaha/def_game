...
로비에서 **벌목 기록 모드(`score_attack`)**와 **무한 디펜스(`defense`)**를 플레이할 수 있다.

```text
나무 6그루로 시작 → 벌목 → 목재 점수와 수종별 정산 재고 누적
활성 나무 0그루 달성 → 재생 단계 영구 상승 · 단계 시간 초기화
40초 뒤 세계수 처치 → 다음 단계 해금 · 클리어 보너스와 목재 즉시 정산 · 로비 복귀
활성 나무가 허용량에 닿으면 즉시 종료 → 결과 정산 → 영구 연구 → 다시 시작
```

고정 제한 시간은 없다. 지는 이유는 언제나 **숲이 다시 차오르는 속도를 못 따라잡아서**다.

흡연자 연구를 모두 만렙으로 채우면 **잡 마스터**가 되어 능력 그대로 자동 전투한다. 다음 기록전부터는 **건설업자**를 새로 키우며 타워크레인으로 건설자재를 굴려 숲을 철거한다. 기존 연구·코인·재생 단계는 유지한다. [잡 마스터 시스템](docs/JOB_MASTER.md)

디펜스는 원형 나무 띠 하나가 한 단계다. 넓어진 전용 맵의 화면 밖 외곽에서 단계당 96그루가 수관을 맞물리며 접근하고, 기존 띠 제거 여부와 관계없이 1.25초마다 체력이 더 높은 다음 단계 띠가 계속 유입된다. 나무는 기본 체력 2배로 시작해 단계마다 25%씩 더 강해지고 초당 48 월드 단위로 중앙을 향한다. 목재 드롭은 없으며 나무 1킬이 즉시 재화 1로 계산된다. 나무가 중앙 방어선에 닿으면 패배하며 승리나 시간 제한은 없다.
...
src/
  ...
  defense_mode.lua      시간 기반 원형 띠 유입·단계 체력·중앙 침범 규칙
